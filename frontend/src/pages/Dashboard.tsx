...
export function Dashboard() {
  ...
  const stats = useMemo(() => {
    ...
    const avgSeverity = scores.length ? Math.round(scores.reduce((a, b) => a + b, 0) / scores.length) : null;
    ...
    return { total: attacks.length, completed, avgSeverity, priorityCounts, allStages };
  }, [attacks, details]);

  return (
    <AppLayout title="Command Center" subtitle="Live overview of simulated attacks and detections">
      <div className="flex flex-col gap-6">
        {/* Hero */}
        <motion.div
          initial={{ opacity: 0, y: 10 }}
          animate={{ opacity: 1, y: 0 }}
          className="relative overflow-hidden rounded-2xl border border-[var(--border-subtle)] bg-gradient-to-br from-[var(--bg-surface)] to-[var(--bg-surface-raised)] p-6 lg:p-8"
        >
          <div
          ...
          <div className="relative flex flex-col lg:flex-row lg:items-center justify-between gap-6">
            <div>
              <span className="inline-flex items-center gap-1.5 text-xs font-medium px-2.5 py-1 rounded-full bg-[var(--accent-cyan-soft)] text-[var(--accent-cyan)] mb-3">
                <span className="w-1.5 h-1.5 rounded-full bg-current animate-pulse-slow" />
                AI-driven SOC console
              </span>
              <h2 className="font-display text-2xl lg:text-3xl font-semibold text-[var(--text-primary)] max-w-lg leading-tight">
                Generate an attack, watch it get detected, analyzed, and reported.
              </h2>
              <p className="text-sm text-[var(--text-muted)] mt-2 max-w-md">
                From MITRE-mapped scenario to IOCs, detection rules, and an executive report - end to end.
              </p>
            </div>
            <Link to="/generate">
              <Button size="lg">
                <BoltIcon /> Generate New Attack
              </Button>
            </Link>
          </div>
        </motion.div>
        ...
        {loading ? (
          ...
          <>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
              <StatCard label="Total attacks" value={stats.total} accent="cyan" icon={<TargetIcon />} />
              <StatCard label="Completed" value={stats.completed} accent="violet" icon={<CheckIcon />} />
              <StatCard
              ...
              />
              <StatCard
                label="Stages analyzed"
                value={stats.allStages.length}
                accent="neutral"
                icon={<LayersIcon />}
              />
            </div>
            ...
            <Card padding="none">
              <div className="p-6 pb-0">
                <CardHeader title="Recent attacks" subtitle="Latest generated scenarios" />
              </div>
